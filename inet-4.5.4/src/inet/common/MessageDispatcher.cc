...
#include "inet/networklayer/contract/IInterfaceTable.h" // For IInterfaceTable (base class)
...
//    EV_INFO << getFullPath() << " initialized at stage " << stage << endl;
//
//    EV_INFO << "MessageDispatcher@" << getFullPath() << " mappings:\n";
//    if (getFullPath() == "EdgeNodeExample.edge1.networkLayer.lp" || getFullPath() == "EdgeNodeExample.edge2.networkLayer.lp" )
//        registerProtocol(Protocol::ethernetMac, gate("in", 0), gate("out", 0));
//    EV_INFO << "Registering ethernetmac protocol on gate index 0: "
//            << gate("in", 0)->getFullPath() << " <--> "
//            << gate("out", 0)->getFullPath() << endl;

    for (auto &entry : serviceToGateIndex) {
        int protoId = entry.first.getProtocolId();
        auto prim   = entry.first.getServicePrimitive();
        EV_INFO << "  protocolId=" << protoId
                << " prim=" << prim
                << " -> gate index=" << entry.second << "\n";
    }
}
