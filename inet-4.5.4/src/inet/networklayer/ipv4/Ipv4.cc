...
const NetworkInterface *Ipv4::getDestInterface(Packet *packet)
{
...
const NetworkInterface *Ipv4::getShortestPathInterfaceToSource(const Ptr<const Ipv4Header>& ipv4Header) const
{
...
void Ipv4::arpResolutionTimedOut(IArp::Notification *entry)
{
    if (entry->l3Address.getType() != L3Address::IPv4)
        return;
    auto ipv4Dest = entry->l3Address.toIpv4();
    auto it = pendingPackets.find(ipv4Dest);
    if (it != pendingPackets.end()) {
        cPacketQueue& packetQueue = it->second;
        ...
        for (int i = 0; i < packetQueue.getLength(); i++) {
            ...
            auto netPkt = check_and_cast<Packet *>(packet);  // OMNeT++/INET packets are usually of type Packet
                       auto protocol = netPkt->getTag<PacketProtocolTag>()->getProtocol();
                       auto destTag = netPkt->findTag<NetworkProtocolInd>();
                       auto srcTag = netPkt->findTag<NetworkProtocolInd>();

                       EV_DEBUG << "  - Dropping packet: " << netPkt->getFullName()
                                << ", size: " << netPkt->getByteLength()
                                << ", protocol: " << (protocol ? protocol->getName() : "unknown")
                                << "\n";

                       if (destTag) {
//                           EV_DEBUG << "    dest: " << destTag->getDe .str() << "\n";
                       }
                       if (srcTag) {
//                           EV_DEBUG << "    src: " << srcTag->getSourceAddress().str() << "\n";
                       }
            PacketDropDetails details;
...
void Ipv4::receiveSignal(cComponent *source, simsignal_t signalID, cObject *obj, cObject *details)
{
    ...
    EV_INFO << "Received signal ID: " << signalID
            << " (fail = 153, 152=success) ; from: " << source->getFullPath() << "\n";
    auto comps = IArp::arpResolutionCompletedSignal;
    auto fails = IArp::arpResolutionFailedSignal;
    if (signalID == comps) {
