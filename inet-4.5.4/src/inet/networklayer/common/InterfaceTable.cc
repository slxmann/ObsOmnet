...
//        std::string debugHostPath = host->getFullPath();
...
{
    cModule *ifmod = omnetpp::check_and_cast<cModule *>(entry);

    if (!ifmod) // returns  if ifmod is nullptr
        return; // virtual interface
    // Store initial paths (debugger-friendly)

    std::string debugEntryPath = entry->cModule::getFullPath();       // returns edg1
    std::string debugHostPath = host->getFullPath();
//    cModule *hosrParent = host->getParentModule();
//    cModule *EntryPArent = ifmod->getParentModule();

    std::vector<std::string> debugTraversalPath;            // Record each step
    ASSERT(host != nullptr);

    //their old code,
//    while (ifmod && ifmod->getParentModule() != host) {
//        ifmod = ifmod->getParentModule(); // THIS RETURNS a NULLPTR  !!!!
//    }

    // ifmod is something like "host.eth[1].mac"; climb up to find "host.eth[1]" from it
    // done by removing the most point to the right and the dir after it
  while(debugEntryPath != debugHostPath)
  {
      size_t lastDot = debugEntryPath.rfind('.');
      if (lastDot == std::string::npos)
          break;  // no dot found, can't trim further

      debugEntryPath = debugEntryPath.substr(0, lastDot);  // remove last module
  }
    ifmod = host;
    if (!ifmod)
...
//    {
//        std::string debugLastValidPath = debugTraversalPath.empty() ? "" : debugTraversalPath.back(); // this is edg1
//        bool hostIsRoot = (host->getParentModule() == nullptr);
//        throw cRuntimeError(
//            "Module hierarchy mismatch!\n"
//            "  Entry: %s\n"
//            "  Host:  %s\n"
//            "  Last valid module: %s\n"
//            "  Host is root: %d",
//            debugEntryPath.c_str(),
//            debugHostPath.c_str(),
//            debugLastValidPath.c_str(),
//            hostIsRoot
//        );
//    }
