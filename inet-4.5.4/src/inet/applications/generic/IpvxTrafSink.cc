...
{
    if (dynamic_cast<OBS_Burst*>(msg))
           processBurst(check_and_cast<OBS_Burst*>(msg));
    else
...
void IpvxTrafSink::processBurst(OBS_Burst *burst)
{
    emit(packetReceivedSignal, burst);
    EV_INFO << "Received burst with " << burst->getMessages() << " messages\n";

//    // Process individual messages in the burst
    while (burst->hasMessages()) {
        cMessage *msg = burst->retrieveMessage();
...
//    delete burst;  this needs checking
    numReceived++;
}
