...
void testSink4::handleMessage(omnetpp::cMessage *msg){
	if (dynamic_cast <OBS_BurstControlPacket *> (msg) != NULL){
		OBS_BurstControlPacket *bcp = (OBS_BurstControlPacket *) msg;
		if (bcp->getKind() == 1){
			outputFile << "1\t" << omnetpp::simTime() << "\t" << bcp->getArrivalGate()->getIndex() << "\t" << bcp->getBurstifierId() << "\t" << bcp->getNumSeq() << "\t" << bcp->getSenderId() << "\t" << bcp->getBurstArrivalDelta() << "\t" << bcp->getBurstColour() << "\t" << bcp->getLabel() << "\t" << bcp->getBurstSize() << "\t" << bcp->getByteLength() << endl;
		}
		else if (bcp->getKind() == 2){
			outputFile << "2\t" << omnetpp::simTime() << "\t" << bcp->getArrivalGate()->getIndex() << "\t" << bcp->getBurstifierId() << "\t" << bcp->getNumSeq() << endl;
		}
		else{
			cout << "<testSink4><" << omnetpp::simTime() << "> WARNING: Received an OBS_BurstControlPacket with an unknown kind." << endl;
		}
	}
	else{
		cout << "<testSink4><" << omnetpp::simTime() << "> WARNING: Received a message which is not an OBS_BurstControlPacket." << endl;
	}
	delete msg;
}
