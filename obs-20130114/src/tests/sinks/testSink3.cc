...
void testSink3::handleMessage(omnetpp::cMessage *msg){
	if (dynamic_cast <OBS_BurstControlPacket *> (msg) != NULL){
		OBS_BurstControlPacket *bcp = (OBS_BurstControlPacket *) msg;
		if (bcp->getKind() == 1){
			outputFile << "1\t" << omnetpp::simTime() << "\t" << bcp->getArrivalGate()->getIndex() << "\t" << bcp->getBurstifierId() << "\t" << bcp->getNumSeq() << "\t" << bcp->getSenderId() << "\t" << bcp->getLabel() << "\t" << bcp->getBurstSize() << "\t" << bcp->getBurstArrivalDelta() << "\t" << bcp->getBurstColour() << "\t" << bcp->getByteLength() << endl;
		}
		else if (bcp->getKind() == 2){
			outputFile << "2\t" << omnetpp::simTime() << "\t" << bcp->getArrivalGate()->getIndex() << "\t" << bcp->getBurstifierId() << "\t" << bcp->getNumSeq() << endl;
		}
		else{
			cout << "<testSink3><" << omnetpp::simTime() << "> WARNING: Received an OBS_BurstControlPacket with an unknown kind." << endl;
		}
	}
		...
		if (burst->getKind() == 1){
			...
			outputFile << "3\t" << omnetpp::simTime() << "\t" << burst->getArrivalGate()->getIndex() << "\t" << bId << "\t" << seq << "\t" << burst->getSenderId() << "\t" << burst->getByteLength() << "\t" << burst->getNumPackets() << "\t" << burst->getMinOffset() << "\t" << burst->getMaxOffset() << endl;
...
//			    IPDatagram *datagram = check_and_cast <IPDatagram *> (burst->retrieveMessage());
			    omnetpp::cMessage *cmsg = burst -> retrieveMessage();
			    auto packet = omnetpp::check_and_cast<inet::Packet*>(cmsg);
			    const auto& iphdr = packet->peekAtFront<inet::Ipv4Header>();
			    protocol = iphdr->getProtocolId();
				if (protocol == inet::IP_PROT_TCP){
	                const auto& tcphdr = packet->peekAtFront<inet::tcp::TcpHeader>();
	                //Save datagram's information in the file
	                auto totalBytes = inet::B(iphdr->getChunkLength()).get(); // total packet size (headers+payload) in bytes == getByteLength();
					outputFile << "P\t" << iphdr->getSrcAddress() << "\t" << iphdr->getDestAddress() << "\t" << bId << "\t" << seq << "\t" << protocol << "\t" << tcphdr->getSrcPort() << "\t" << tcphdr->getDestPort() << "\t" <<totalBytes << endl;
					//delete segment;
				}
				else if (protocol == inet::IP_PROT_UDP){
				    const auto& udphdr = packet->peekAtFront<inet::UdpHeader>();
				    //Save datagram's information in the file
				    auto totalBytes = inet::B(iphdr->getChunkLength()).get(); // total packet size (headers+payload) in bytes == getByteLength();
					outputFile << "P\t" << iphdr->getSrcAddress() << "\t" << iphdr->getDestAddress() << "\t" << bId << "\t" << seq << "\t" << protocol << "\t" << udphdr->getSourcePort() << "\t" << udphdr->getDestinationPort() << "\t" << totalBytes << endl;
					//delete packet;
				}
				else{
					cout << "<testSink3><" << omnetpp::simTime() << "> WARNING: Received a packet with a protocol different from TCP or UDP." << endl;
                    auto totalBytes = inet::B(iphdr->getChunkLength()).get(); // total packet size (headers+payload) in bytes == getByteLength();

					outputFile << "P\t" << iphdr->getSrcAddress() << "\t" << iphdr->getDestAddress() << "\t" << bId << "\t" << seq << "\t" << protocol << "\t" << "-1" << "\t" << "-1" << "\t" << totalBytes << endl;
				}
				delete packet;
			}
		}
			...
			//Save burst's information in the file
			outputFile << "4\t" << omnetpp::simTime() << "\t" << burst->getArrivalGate()->getIndex() << "\t" << burst->getBurstifierId() << "\t" << burst->getNumSeq() << endl;
		}
		else{
			cout << "<testSink3><" << omnetpp::simTime() << "> WARNING: Received an OBS_Burst with an unknown kind." << endl;
		}
	}
	else{
		cout << "<testSink3><" << omnetpp::simTime() << "> WARNING: Received a message which is neither an OBS_BurstControlPacket nor an OBS_Burst." << endl;
	}
	delete msg;
}
	...
	if (OK){
		...
		if (par("compareReport")){
			...
			if(modulo->hasPar("reportFile")){
				currentReport.open(modulo->par("reportFile").stringValue());
			}
			else{
			    omnetpp::cRuntimeError("The tested module has not a parameter named 'reportFile'");
			}

			if (!currentReport.is_open())
			    omnetpp::cRuntimeError("Cannot open the report");
			patternReport.open(par("reportPattern").stringValue());
			if (!patternReport.is_open())
			    omnetpp::cRuntimeError("Cannot open the pattern of the report");
		...
		while(OK && patternFile.good() && currentFile.good()){
			...
			//3. Compare iniBurst output
				...
				if(strcmp(minOffsetPChr,"*") != 0){ //Compare this parameter
					//Convert from char * to SimTime
				    omnetpp::simtime_t minOffsetP = omnetpp::SimTime::parse(minOffsetPChr);
				    omnetpp::simtime_t minOffset = omnetpp::SimTime::parse(minOffsetChr);
				...
				if(strcmp(maxOffsetPChr,"*") != 0){ //Compare this parameter
					//Convert from char * to SimTime
				    omnetpp::simtime_t maxOffsetP = omnetpp::SimTime::parse(maxOffsetPChr);
				    omnetpp::simtime_t maxOffset = omnetpp::SimTime::parse(maxOffsetChr);
...
//					IPAddress destAddr;
			    ...
			    omnetpp::cRuntimeError("Pattern file: Unknown beginning character on line %d", line);
			}
		}
