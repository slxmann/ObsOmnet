...
void testSink2::handleMessage(omnetpp::cMessage *msg){
	if ((dynamic_cast <OBS_Burst *> (msg) != NULL)&&(dynamic_cast <OBS_BurstifierInfo *> (msg->getControlInfo()) != NULL)){
		...
		outputFile << omnetpp::simTime() << "\t" << bId << "\t" << seq << "\t" << burst->getNumPackets() << "\t" << burst->getMinOffset() << "\t" << burst->getMaxOffset() << "\t" << info->getLabel() << "\t" << burst->getByteLength() << endl;
...
//				TCPSegment *segment = check_and_cast <TCPSegment *> (datagram->getEncapsulatedPacket());
				...
				outputFile << "P\t" << bId << "\t" << seq << "\t" << iphdr->getSrcAddress() << "\t" << iphdr->getDestAddress() << "\t" << protocol << "\t" << tcphdr->getSrcPort() << "\t" << tcphdr->getDestPort() << "\t" << totalBytes << endl;
				//delete segment;
			}
...
//				UDPPacket *packet = check_and_cast <UDPPacket *> (datagram->getEncapsulatedPacket());
			    ...
			    auto totalBytes = inet::B(iphdr->getChunkLength()).get(); // total packet size (headers+payload) in bytes == getByteLength();
				//Save datagram's information in the file
				outputFile << "P\t" << bId << "\t" << seq << "\t" << iphdr->getSrcAddress() << "\t" << iphdr->getDestAddress() << "\t" << protocol << "\t" << udphdr->getSourcePort() << "\t" << udphdr->getDestinationPort() << "\t" << totalBytes<< endl;
				//delete packet;
			}
			else{
				cout << "<testSink2><" << omnetpp::simTime() << "> WARNING: Received a packet with a protocol different from TCP or UDP." << endl;
                auto totalBytes = inet::B(iphdr->getChunkLength()).get(); // total packet size (headers+payload) in bytes == getByteLength();
				outputFile << "P\t" << bId << "\t" << seq << "\t" << iphdr->getSrcAddress() << "\t" << iphdr->getDestAddress() << "\t" << protocol << "\t" << "-1" << "\t" << "-1" << "\t" << totalBytes << endl;
			}
			delete packet;
		}
	}
	else{
		cout << "<testSink2><" << omnetpp::simTime() << "> WARNING: Received a message which is not an OBS_Burst." << endl;
	}
	delete msg;
}
...
void testSink2::openFiles(){
	...
	if (compType == 1 || compType == 2){
		//Open both files in read mode
		currentFile.open(par("outputFile").stdstringValue());
