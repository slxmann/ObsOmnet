...
void testSource5::handleMessage(omnetpp::cMessage *msg){
...
void testSource5::processLine(){
	...
	omnetpp::simtime_t sendTime, burstArrival, BCPArrival;
	...
	if (inputFile.good()){//If there is no problem and the end of file is not reached, get BCP's information
			...
			if(omnetpp::simTime()<=sendTime){//If the bcp is not scheduled to the past, schedule it
				...
				burstArrival = omnetpp::SimTime::parse(burstArrivalChr);
				...
				info->setPort(port);
				if(strcmp(sizeChr,"*") != 0){ //Optional parameter
					BCPArrival = omnetpp::SimTime::parse(BCPArrivalChr);
