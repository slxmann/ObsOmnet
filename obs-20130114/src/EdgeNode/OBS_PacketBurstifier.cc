...
void OBS_PacketBurstifier::initialize(){
   ...
   timeout_msg = new omnetpp::cMessage("timeout");
...
void OBS_PacketBurstifier::handleMessage(omnetpp::cMessage *msg){
...
//       inet::Ipv4Header *arrivedMsg = omnetpp::check_and_cast < inet::Ipv4Header *> (msg);
       inet::Packet *packet = omnetpp::check_and_cast<inet::Packet*>(msg);
       const auto& ipv4Header = packet->peekAtFront<inet::Ipv4Header>();
       auto lenInBits  = packet->getTotalLength().get();  // bits
       auto lenInBytes = lenInBits / 8;

       recvPackSizeVec.record(lenInBytes);
       recvPackSize.collect(lenInBytes);
...
//      recvPackSize.collect(arrivedMsg->getByteLength());
      ...
      //If queue is empty at this moment, display an error message because something weird happened :S
         if(burstContent.isEmpty())
             omnetpp::cRuntimeError("Attempted to assemble a burst using an empty queue");
      //Assemble burst and restart counters
         ...
         scheduleAt(omnetpp::simTime()+timeout,timeout_msg);
         firstPacket_t = omnetpp::simTime();
         //Calculate if overflow happens with the first message
         overflowHappened = false;
         if(((burstBits + lenInBits + varHeaderInBits) > maxSizeInBits)) overflowHappened = true;
      } 
      //Anyway, insert the current message into the queue
      queuedTime.collect(omnetpp::simTime() - firstPacket_t);

      burstContent.insert(msg);
      burstBits += lenInBits + varHeaderInBits;
      numBurstPackets++;

      //If burst overflow is not allowed, but overflow happens when you insert the first message, show an error message
      if(overflowHappened && !overflowLastPacket)
      {
          omnetpp::cRuntimeError("Queue overflow happened inserting the first message and you don't allow Queue overflow (overflowLastPacket is false)");
      }
...
void OBS_PacketBurstifier::assembleBurst(){
   ...
   avgPacketTime.record((omnetpp::simTime() - firstPacket_t) - queuedTime.getMean());
...
//   wrapper->setPointerValue(info);
   ...
   burstCounter++;
   omnetpp::cMessage *tempMsg;
   while(!(burstContent.isEmpty())){
      tempMsg = (omnetpp::cMessage*)burstContent.pop();
