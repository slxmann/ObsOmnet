...
class OBS_DropBurst : public  omnetpp::cSimpleModule
{
   protected:
...
void OBS_DropBurst::handleMessage( omnetpp::cMessage *msg){
    omnetpp::cGate *gate = msg->getArrivalGate(); //Get arrival gate

   if(dynamic_cast<OBS_BurstControlPacket*>(msg) != NULL && msg->getKind() == 1){ //If it's a BCP beginning
      OBS_BurstControlPacket *bcpIni =  omnetpp::check_and_cast<OBS_BurstControlPacket*>(msg);
      ...
      if(dynamic_cast<OBS_BurstControlPacket*>(msg) != NULL){ //BCP end received
         ...
         OBS_BurstControlPacket *bcpEnd =  omnetpp::check_and_cast<OBS_BurstControlPacket*>(msg);
