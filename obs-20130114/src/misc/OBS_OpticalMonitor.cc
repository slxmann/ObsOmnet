...
void OBS_OpticalMonitor::handleMessage( omnetpp::cMessage *msg){
   //store arrival gate
    omnetpp::cGate *gate = msg->getArrivalGate();
   //don't forget to forward incoming messages

   //check if msg is a BCP or a Burst
   if(dynamic_cast< OBS_Burst *> (msg) != NULL){
      OBS_Burst *b =  omnetpp::check_and_cast < OBS_Burst *> (msg);
      //check if kind=1 (Burst beginning)
      if(b->getKind() == 1){
         //register it!
    	 fprintf(data_f,"1 %s %d %d %ld\n", omnetpp::simTime().str().c_str(),b->getBurstifierId(),b->getNumSeq(),b->getByteLength());
      }
   }else if(dynamic_cast< OBS_BurstControlPacket *> (msg) != NULL){
      OBS_BurstControlPacket *c =  omnetpp::check_and_cast < OBS_BurstControlPacket *> (msg);
      if(c->getKind() == 1)
         //register BCP ini info
    	 fprintf(data_f,"2 %s %d %d %d\n", omnetpp::simTime().str().c_str(),c->getBurstifierId(),c->getNumSeq(),c->getBurstColour());
   }
   //forward everything
