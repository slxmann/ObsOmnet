...
class OBS_Burst : public OBS_Burst_Base
 {
    protected:
    // this is where you *declare* messages_var
    omnetpp::cQueue messages_var{"burstQueue"};
     public:
...
//     OBS_Burst(const OBS_Burst& other) : OBS_Burst_Base(other.getName()) {operator=(other);}
     ...
     virtual void insertMessage( omnetpp::cMessage *msg); //!< Inserts a message in the queue.
     virtual  omnetpp::cMessage* retrieveMessage(); //!< Retrieves and deletes the first message of the queue. NULL if queue is empty.
