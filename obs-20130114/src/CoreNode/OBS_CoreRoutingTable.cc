...
OBS_CoreRoutingTableEntry *OBS_CoreRoutingTable::getEntry(int inPort,int inColour,int inLabel){
   ...
   for(cQueue::Iterator iter_list(routingTable,false) ; !iter_list.end() ; ++iter_list){
       item = check_and_cast<OBS_CoreRoutingTableEntry*> (*iter_list);

      if((item->getInPort() == inPort) &&
        ((item->getInColour() == inColour) || (item->getInColour() == -9)) &&
        ((item->getInLabel() == inLabel) || (item->getInLabel() == -9)))
          return item->dup();
   }
   return NULL;
}
