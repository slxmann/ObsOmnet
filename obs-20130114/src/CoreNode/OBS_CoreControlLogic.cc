...
void OBS_CoreControlLogic::initialize(){
   ...
   routingTable = omnetpp::check_and_cast<OBS_CoreRoutingTable*>(parent->getSubmodule("RoutingTable"));
   gatesHorizon = omnetpp::check_and_cast<OBS_CoreOutputHorizon*>(parent->getSubmodule("GatesHorizon"));
   coreInput = omnetpp::check_and_cast<OBS_CoreInput*>((parent->getParentModule())->getSubmodule("Input"));
   coreOutput = omnetpp::check_and_cast<OBS_CoreOutput*>((parent->getParentModule())->getSubmodule("Output"));
   oxc = omnetpp::check_and_cast<OBS_OpticalCrossConnect*>((parent->getParentModule())->getSubmodule("OXC"));

   EV << "Routing table pointer: " << routingTable << " in module: " << getParentModule()->getFullPath() << endl;
...
void OBS_CoreControlLogic::handleMessage(omnetpp::cMessage *msg){
    std::stringstream ss;
    ss << "Msg name: " << msg->getName()
       << ", Kind: " << msg->getKind()
       << ", Arrival time: " << msg->getArrivalTime();
    std::string msgInfo = ss.str();
   //OXC Programming automessage. If msg has kind = OBS_SCHEDULE_OXC
   if(msg->getKind() == OBS_SCHEDULE_OXC){
      OBS_ControlUnitInfo *info = omnetpp::check_and_cast <OBS_ControlUnitInfo*>(msg);
   ...
   //Step 1 - BCP received
   OBS_BurstControlPacket *bcp = omnetpp::check_and_cast <OBS_BurstControlPacket*>(msg);
   ...
   EV << "BCP arrived at " << getFullPath() << " with inPort=" << arrivalPort << " burstColour=" << burstColour << " destLabel=" << destLabel << endl;
   ...
   simtime_t burstArrival = omnetpp::simTime() + arrivalDelta;
   ...
   if(burstArrival <omnetpp::simTime()){
	//BCP discarded. Burst was here and discarded too.
        ...
        if (data_f != NULL){
            fprintf(data_f,"%d %d %s %s 1 1\n", burstifierId, numSeq, omnetpp::simTime().str().c_str(), burstArrival.str().c_str());
        }
   ...
   //Query Core Node routing table asking for output port,colour and label <inPort> <inColour> <inLabel>
   OBS_CoreRoutingTableEntry *result = routingTable->getEntry(arrivalPort,burstColour,destLabel);
   if(result == NULL)
       omnetpp::cRuntimeError("Error in routing table query (Control Unit id: %d) module %s",getId(),getFullPath().c_str() );
